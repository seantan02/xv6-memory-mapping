...
int duplicatePage(struct proc *p, uint addr, int length); 
int handle_non_wmap_page_fault(struct proc *p, uint fault_addr);
void ref_count_init(void);
void inc_page_ref(uint pa);
int dec_page_ref(uint pa);
unsigned char get_page_ref_count(uint pa);
void reset_page_ref(uint pa);
void initcowrefcounts();
